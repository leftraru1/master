...
var handlePageStates = function () {
    ...
    function handleGenericEvents() {
        ...
        $('#tbFiles tbody').on("click", ".downloadFile", function () {
            var url = initApp.pathBase + "/Informes/DescargarInforme?nombreArchivo=" + $(this).attr("data-id");
            window.open(url, '_blank');
        });

        $(".downloadSelected").on("click", function (e) {
            $.each($("input[name='chkDownloadFile']:checked"), function () {
                var url = initApp.pathBase + "/Informes/DescargarInforme?nombreArchivo=" + $(this).attr("data-id");
                window.open(url, '_blank');
            });
        });
    ...
    return {
        ...
        llenarData: function (data) {
            ...
            $.each(data, function (i, elem) {
                tb.row.add([
                    ""
                    , "<input type='checkbox' name='chkDownloadFile' value='false' class='checkDownload' data-id='" + elem.NOMBRE_ARCHIVO + "'>"
                    , "<a href='javascript:void(0);' data-id='" + elem.NOMBRE_ARCHIVO + "' class='fa fa-download fa-lg downloadFile' data-toggle='tooltip'   title='Descargar Informe' data-original-title='Descargar Informe' ></a>"
                    , elem.NOMBRE_USUARIO
                    , elem.NUMERO_INFORME
...
var handleServicio = function () {
    ...
    function filtros() {
        var file = {
            NUMERO_INFORME: ($("#txtNumeroInforme").val() == "") ? null : $("#txtNumeroInforme").val(),
            NOMBRE_USUARIO: ($("#txtNombreUsuario").val() == "") ? null : $("#txtNombreUsuario").val(),
